...
export async function addPlayerToSet(
  ...
  try {
    ...
    // Use SADD to add player to set (automatically handles duplicates)
...
export async function storePlayerGuess(
  ...
  try {
    ...
    throw new Error(`Failed to store player guess: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getAggregatedGuesses(
  ...
  try {
    ...
    throw new Error(`Failed to get aggregated guesses: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getTotalPlayers(
  ...
  try {
    ...
    // Use SCARD to count unique players
    const count = await redis.sCard(key);
    
    return count;
  } catch (error) {
    console.error(`Error getting total players for prompt ${promptId}:`, error);
    throw new Error(`Failed to get total players: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
